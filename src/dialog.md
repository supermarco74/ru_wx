...
lighter [`message_box`](./message_box.md) helper.

## See Also
- [`frame.rs`](./frame.md) — parent window pattern; a `Dialog` is always created with a `&Frame` reference
- [`message_box.rs`](./message_box.md) — lighter alternative for simple OK/Cancel prompts
- [`message_dialog.rs`](./message_dialog.md) — class-based wrapper around `MessageBoxW`
- [`file_dialog.rs`](./file_dialog.md) — specialised dialog for file picking
- [`button.rs`](./button.md) — `create_button` returns a `WidgetRef`; the `Button` API applies to the returned widget

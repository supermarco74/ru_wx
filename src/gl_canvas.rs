...
#[cfg(target_os = "windows")]
impl Drop for GLCtxInner {
    fn drop(&mut self) {
        // Release the GL context from the current thread before
        // destroying it, otherwise a context that is current on a
        // dead thread will be leaked.
        if !self.hglrc.is_null() {
            // SAFETY: passing null HDC/HGLRC releases the
            // current context. Safe to call even if no context is
            // current.
            unsafe {
        ...
        if !self.hwnd.is_null() && !self.hdc.is_null() {
            // SAFETY: `self.hdc` was acquired with `GetDC` in
            // `with_size`; we release it here.
            unsafe {
...
unsafe extern "system" fn gl_canvas_wnd_proc(
    ...
    match msg {
        WM_DESTROY => {
            ...
            if ptr != 0 {
                let _ = Rc::from_raw(ptr as *const RefCell<GLCtxInner>);
...
mod tests {
    ...
    fn default_size_is_positive() {
        assert!(GLCanvas::DEFAULT_W > 0);
        assert!(GLCanvas::DEFAULT_H > 0);
    }

...
macro_rules! wx_log_warning {
    ($($arg:tt)*) => {
        $crate::core::log::log_message(
            $crate::core::log::LogLevel::Warning,
            "",
            format!($($arg)*),
        );
    };
...
macro_rules! wx_log_message {
    ($($arg:tt)*) => {
        $crate::core::log::log_message(
            $crate::core::log::LogLevel::Message,
            "",
            format!($($arg)*),
        );
    };
...
macro_rules! wx_log_sys_error {
    ($($arg:tt)*) => {
        $crate::core::log::log_win32_error(&format!($($arg)*));
    };
}
...
macro_rules! wx_api_block {
    ($name:expr, $body:block) => {{
        let _api_guard = $crate::core::log::ApiGuard::new($name);
        $body
    }};
}

...
impl Timer {
    ...
    pub fn new(frame: &Frame) -> Self {
        ...
        frame.register_timer_tick_route(timer_id as usize, msg_id);
    ...
    /// Tick with [`TimerEvent`] payload (`wxTimerEvent`).
    pub fn on_timer_event<F: FnMut(&crate::TimerEvent) + 'static>(&self, mut f: F) {
        let id = self.state.borrow().timer_id;
        self.on_tick(move || f(&crate::TimerEvent::new(id)));
    }
...
impl Drop for Timer {
    fn drop(&mut self) {
        ...
        self.frame.unregister_timer_tick_route(timer_id as usize);
    }
}
...
impl Timer {
    ...
    pub fn on_timer_event<F: FnMut(&crate::TimerEvent) + 'static>(&self, _f: F) {}

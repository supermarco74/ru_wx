...
impl ToggleButton {
    ...
    /// State change with [`ToggleButtonEvent`] payload (`wxToggleButtonEvent`).
    pub fn on_toggle_event<F: FnMut(&crate::ToggleButtonEvent) + 'static>(
        &self,
        frame: &Frame,
        mut f: F,
    ) {
        self.on_toggle(frame, move |pressed| {
            f(&crate::ToggleButtonEvent::new(pressed));
        });
    }

...
//! class `msctls_updown32`. Unlike [`crate::controls::spin_ctrl::SpinCtrl`]
...
#[cfg(target_os = "windows")]
impl Drop for SpinButton {
    fn drop(&mut self) {
        if Rc::strong_count(&self.inner) == 1 {
            let inner = self.inner.borrow();
            if let Some(ref frame) = inner.notify_frame {
                frame.unregister_notify_handler(inner.id);
            }
        }
    }
}

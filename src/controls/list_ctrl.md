...
## See Also
- [`frame.rs`](../window/frame.md) — provides `register_notify_handler` / `register_disp_info_handler` used by `on_item_selected` and `on_get_disp_info`.
- [`list_box.rs`](list_box.md) — simpler LISTBOX primitive, single-column only.
- [`widget.rs`](../core/widget.md) — `Widget` trait implementation; `as_widget_ref` and sizer integration.

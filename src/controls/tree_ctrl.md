...
- [frame](../window/frame.md) — `TreeCtrl::new` takes a `Frame`; `on_selection_change` registers a
  notify handler on the frame.
- [widget](../core/widget.md) — `as_widget_ref()` for use with sizers.
- [sizer](../containers/sizer.md) — typical layout is "tree on the left, content on the right" inside a

...
impl RadioBox {
    ...
    /// Selection with [`RadioBoxEvent`] payload (`wxRadioBoxEvent`).
    pub fn on_radio_event<F: FnMut(&crate::RadioBoxEvent) + 'static>(
        &self,
        frame: &Frame,
        mut f: F,
    ) {
        self.on_select(frame, move |sel| f(&crate::RadioBoxEvent::new(sel)));
    }

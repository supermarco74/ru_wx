...
//! [`crate::animation::Animation`] and advances its frames at the
...
unsafe extern "system" fn animation_ctrl_wnd_proc(
    ...
    match msg {
        ...
        WM_TIMER => {
            if wparam as usize != ANIMATION_TIMER_ID {
...
mod tests {
    ...
    fn default_size_is_positive() {
        assert!(AnimationCtrl::DEFAULT_W > 0);
        assert!(AnimationCtrl::DEFAULT_H > 0);
    }

...
- [frame](frame.md) — owns the sizer, calls `sizer.layout` on `WM_SIZE`.
- [widget](widget.md) — `WidgetRef` and the `Widget` trait that `BoxSizer` calls into.
- [grid_sizer](grid_sizer.md) — the 2D counterpart.
- [status_bar](status_bar.md) — non-sizer sibling; pair with `add_spacer(STATUS_BAR_HEIGHT)`.
- [prelude](prelude.md)

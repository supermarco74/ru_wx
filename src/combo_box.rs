...
mod bitmap_combo_box_win32 {
    ...
    /// `CBEM_SETIMAGELIST` — attaches an `HIMAGELIST` to the control.
    pub const CBEM_SETIMAGELIST: u32 = 0x0540;
    /// `CBEM_INSERTITEM` — inserts a new row with an image and a text.
    pub const CBEM_INSERTITEM: u32 = 0x0541;
    /// `CBEM_GETIMAGELIST` — returns the attached `HIMAGELIST`.
    pub const CBEM_GETIMAGELIST: u32 = 0x0542;
    /// `CBEM_SETITEM` — updates the image / text of an existing row.
    pub const CBEM_SETITEM: u32 = 0x0544;
    /// `CBEM_DELETEITEM` — removes a row.
    pub const CBEM_DELETEITEM: u32 = 0x0546;
    /// `CBEM_GETCOUNT` — returns the row count.
    pub const CBEM_GETCOUNT: u32 = 0x0548;
    /// `CBEM_SETCURSEL` — sets the selected row.
    pub const CBEM_SETCURSEL: u32 = 0x054E;
    /// `CBEM_GETCURSEL` — returns the selected row (or `CB_ERR`).
    pub const CBEM_GETCURSEL: u32 = 0x0547;
    /// `CBEM_SETEXTENDEDSTYLE` — toggles the extended styles.
    pub const CBEM_SETEXTENDEDSTYLE: u32 = 0x054F;
...
mod tests {
    ...
    fn win32_constants_pinned() {
        // CBEM_* messages (comboboxex.h)
        assert_eq!(CBEM_SETIMAGELIST, 0x0540);
        assert_eq!(CBEM_INSERTITEM, 0x0541);
        assert_eq!(CBEM_GETIMAGELIST, 0x0542);
        assert_eq!(CBEM_SETITEM, 0x0544);
        assert_eq!(CBEM_DELETEITEM, 0x0546);
        assert_eq!(CBEM_GETCURSEL, 0x0547);
        assert_eq!(CBEM_GETCOUNT, 0x0548);
        assert_eq!(CBEM_SETCURSEL, 0x054E);
        assert_eq!(CBEM_SETEXTENDEDSTYLE, 0x054F);

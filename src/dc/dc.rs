...
use crate::dc::brush::Brush;
use crate::core::geometry::{Colour, Rect};
use crate::dc::pen::Pen;
...
/// Device context for the entire screen (`wxScreenDC`).
pub struct ScreenDC {
    #[cfg(target_os = "windows")]
    hdc: HDC,
}
...
impl ScreenDC {
    pub fn new() -> Self {
        #[cfg(target_os = "windows")]
        // SAFETY: `GetDC(NULL)` returns a screen DC valid until `ReleaseDC`.
        unsafe {
            Self {
                hdc: GetDC(0 as HWND),
            }
...
impl Drop for ScreenDC {
    fn drop(&mut self) {
        unsafe {
            let _ = ReleaseDC(0 as HWND, self.hdc);
        }
    }
}

impl Dc for ScreenDC {
    fn handle(&self) -> isize {
        #[cfg(target_os = "windows")]
        {
            self.hdc as isize
    ...
    fn draw_text_in_rect(&mut self, text: &str, rect: Rect, centre: bool) {
        ...
        let flags = if centre { 0x00000001 | 0x00000020 } else { 0 };
        unsafe {
            let _ = DrawTextW(
                self.hdc,
                wide.as_ptr(),

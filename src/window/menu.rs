...
//! frame with [`crate::window::frame::Frame::set_menu_bar`]. Use
//! [`crate::window::popup_menu::PopupMenu`] for context menus invoked from a
//! mouse event.
...
use windows_sys::Win32::UI::WindowsAndMessaging::{
    ...
    GetMenuState, ModifyMenuW, PostMessageW, SetForegroundWindow, SetMenuItemBitmaps,
    TrackPopupMenu, HMENU, MF_BYCOMMAND, MF_CHECKED, MF_GRAYED, MF_POPUP, MF_STRING,
    TPM_BOTTOMALIGN, TPM_RIGHTBUTTON, WM_NULL,
};
...
pub struct Menu {
    ...
    #[cfg(target_os = "windows")]
    bitmaps: Vec<HBITMAP>,
    /// `true` once the menu's `HMENU` has been handed to a parent
    /// (`MenuBar::append` via `AppendMenuW(MF_POPUP, …)`). The
    /// parent HMENU then owns the submenu and destroys it
    /// recursively, so [`Menu::drop`] must not call `DestroyMenu`
    /// itself (that would be a double free).
    attached: bool,
}
...
impl Menu {
    ...
    pub fn append_with_svg_icon<F: FnMut() + 'static>(
        ...
        unsafe {
            ...
            if let Some(hbmp) = crate::dc::icon::svg_bytes_to_hbitmap(svg_bytes, icon_size, icon_size) {
                SetMenuItemBitmaps(self.hmenu, id as u32, MF_BYCOMMAND, hbmp, hbmp);
                self.bitmaps.push(hbmp);
            }
        }
...
impl Drop for Menu {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            // Free-standing popups (context menus, menus never
            // appended to a bar) own their HMENU and must release
            // it; submenus appended to a `MenuBar` are destroyed
            // recursively by the bar's own `DestroyMenu`.
            if !self.attached && !self.hmenu.is_null() {
...
pub struct MenuBar {
    ...
    hmenu: HMENU,
    menus: Vec<Menu>,
    /// `true` once the bar has been installed on a window with
    /// `SetMenu` (see [`crate::Frame::set_menu_bar`]). The window
    /// then owns the HMENU and destroys it on `WM_DESTROY`, so
    /// [`MenuBar::drop`] must not call `DestroyMenu` itself.
    attached: bool,
}
...
impl MenuBar {
    ...
    pub fn append(&mut self, mut menu: Menu) {
        #[cfg(target_os = "windows")]
        // SAFETY: Win32 FFI call with validated arguments (HWND / HMENU / handle) and a buffer large enough for the output.
        unsafe {
        ...
        // Ownership of the submenu's HMENU transfers to the bar
        // (destroying the bar destroys submenus recursively), so
        // the `Menu`'s own Drop must no longer call `DestroyMenu`.
        menu.attached = true;
        self.menus.push(menu);
    }

    /// Mark the bar as owned by a window (`SetMenu` was called) or
    /// re-orphaned (replaced by another bar) so `Drop` knows
    /// whether it must release the HMENU itself.
...
impl Drop for MenuBar {
    fn drop(&mut self) {
        #[cfg(target_os = "windows")]
        // SAFETY: `self.hmenu` was created by `CreateMenu` in
        // `MenuBar::new`. `DestroyMenu` recursively destroys the
        // appended submenus too — their `Menu::drop` skips its own
        // `DestroyMenu` because `MenuBar::append` marked them
        // `attached`, so there is no double free.
        unsafe {
            if !self.attached && !self.hmenu.is_null() {
                DestroyMenu(self.hmenu);
            }

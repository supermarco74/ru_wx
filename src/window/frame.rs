...
/// List of event callbacks taking a shared event reference; used
/// for every `on_*` event field of [`FrameData`].
pub(crate) type EventHandlers<E> = Vec<Box<dyn FnMut(&E)>>;

/// Single optional event callback taking a shared event reference.
pub(crate) type EventHandlerSlot<E> = Option<Box<dyn FnMut(&E)>>;

/// List of event callbacks taking a mutable event reference (the
...
pub(crate) struct FrameData {
    ...
    /// where the notification code is `HIWORD(wparam)` (e.g.
    /// `LBN_SELCHANGE` vs `LBN_DBLCLK`). Looked up before the
    /// id-only `command_handlers` map.
    pub command_notify_handlers: HashMap<(u16, u16), Box<dyn FnMut()>>,
    ...
    /// Handlers invoked when a child toolbar (or other control) sends
    /// `TTN_GETDISPINFOW` to request tooltip text. Keyed by the
    /// control's `idFrom` (the toolbar child id).
    pub ttn_dispinfo_handlers: HashMap<u16, Box<dyn FnMut(isize)>>,
    /// Handlers invoked when a child `SysListView32` (e.g. [`crate::Grid`])
    /// dispatches `NM_CUSTOMDRAW`. Keyed by the control's `idFrom`.
    /// The handler receives the full `lparam` (`NMLVCUSTOMDRAW`) and
    ...
    /// Maps Win32 `SetTimer` ids (`WM_TIMER`'s `wparam`) to the
    /// `WM_APP + 0x100 + n` message used by [`crate::Timer`].
    pub timer_tick_routes: HashMap<usize, u32>,
    ...
    /// labels (via [`crate::window::menu::MenuBar::update_item_shortcut`])
    ...
    /// Called before destroy; handler may [`crate::CloseEvent::veto`].
    pub on_query_close: EventHandlerSlot<crate::CloseEvent>,
    pub on_key_down: EventHandlers<crate::KeyEvent>,
    pub on_key_up: EventHandlers<crate::KeyEvent>,
    pub on_mouse: EventHandlers<crate::MouseEvent>,
    pub on_mouse_wheel: EventHandlers<crate::MouseWheelEvent>,
    pub on_focus: EventHandlers<crate::FocusEvent>,
    pub on_size_event: EventHandlers<crate::SizeEvent>,
    pub on_move_event: EventHandlers<crate::MoveEvent>,
    pub on_idle: MutEventHandlers<crate::IdleEvent>,
    pub on_activate: EventHandlers<crate::ActivateEvent>,
    pub on_char: EventHandlers<crate::CharEvent>,
    pub on_char_hook: EventHandlers<crate::CharHookEvent>,
    pub on_erase: EventHandlerSlot<crate::EraseEvent>,
    pub on_show: EventHandlers<crate::ShowEvent>,
    pub on_hide: EventHandlers<crate::HideEvent>,
    pub on_iconize: EventHandlers<crate::IconizeEvent>,
    pub on_maximize: EventHandlers<crate::MaximizeEvent>,
    pub on_mouse_enter: EventHandlers<crate::MouseEnterEvent>,
    pub on_mouse_leave: EventHandlers<crate::MouseLeaveEvent>,
    pub on_capture_lost: EventHandlers<crate::MouseCaptureLostEvent>,
    pub on_context_menu: EventHandlers<crate::ContextMenuEvent>,
    pub on_drop_files_event: EventHandlers<crate::DropFilesEvent>,
    pub on_sys_colour_changed: EventHandlers<crate::SysColourChangedEvent>,
    pub on_dpi_changed: EventHandlers<crate::DpiChangedEvent>,
    /// Set after `TrackMouseEvent` until `WM_MOUSELEAVE`.
    pub mouse_tracking: bool,
    /// When `true` (the default), destroying this frame posts
    /// `PostQuitMessage` and ends the application message loop.
    /// Set to `false` on auxiliary top-level windows (e.g. MDI
    /// children) that must not terminate the whole app.
    pub quit_on_destroy: bool,
...
impl Frame {
    ...
    /// Register a handler for a specific `WM_COMMAND` notification
    /// code (`HIWORD(wparam)`), e.g. `LBN_SELCHANGE` (1) vs
    /// `LBN_DBLCLK` (2) on the same control id.
    pub fn register_command_notify_handler(
        &self,
        id: u16,
        notify_code: u16,
        ...
        self.inner
            .borrow_mut()
            .command_notify_handlers
            .insert((id, notify_code), handler);
    }
    ...
    /// Remove a previously-registered `WM_NOTIFY` handler.
    pub fn unregister_notify_handler(&self, id: u16) {
        self.inner.borrow_mut().notify_handlers.remove(&id);
    }
    ...
    /// Register a tooltip-text handler for a child control that
    /// requests strings via `TTN_GETDISPINFOW` (e.g. a
    /// `ToolbarWindow32` button whose `iString` is
    /// `LPSTR_TEXTCALLBACK`). The handler receives the full `lparam`
    /// (`NMTTDISPINFOW`) and must fill `szText` / `lpszText`.
    pub fn register_ttn_dispinfo_handler(&self, id: u16, handler: Box<dyn FnMut(isize)>) {
        self.inner
            .borrow_mut()
            .ttn_dispinfo_handlers
            .insert(id, handler);
    }

    /// Register a custom-draw handler for a child `SysListView32`
    /// (e.g. [`crate::Grid`]). Invoked on `NM_CUSTOMDRAW`; the
    /// handler must return the `CDRF_*` result dword.
    pub fn register_lv_custom_draw_handler(
        &self,
        id: u16,
        handler: Box<dyn FnMut(isize) -> u32>,
    ) {
        self.inner
            .borrow_mut()
            .lv_custom_draw_handlers
            .insert(id, handler);
    }
    ...
    /// Map a Win32 `SetTimer` id to a [`crate::Timer`] tick message.
    pub(crate) fn register_timer_tick_route(&self, timer_id: usize, msg_id: u32) {
        self.inner
            .borrow_mut()
            .timer_tick_routes
            .insert(timer_id, msg_id);
    }

    /// Remove a [`crate::Timer`] tick route (see [`Self::register_timer_tick_route`]).
    pub(crate) fn unregister_timer_tick_route(&self, timer_id: usize) {
        self.inner.borrow_mut().timer_tick_routes.remove(&timer_id);
    }
    ...
    pub fn set_menu_bar(&self, menubar: MenuBar) {
        ...
        unsafe {
            let hwnd = self.inner.borrow().hwnd;
            // A previously installed bar is detached by this SetMenu
            // call: mark it so its Drop releases the orphaned HMENU.
            if let Some(mut old) = self.inner.borrow_mut().menu_bar.take() {
                old.set_attached(false);
            }
            SetMenu(hwnd, menubar.hmenu());
            DrawMenuBar(hwnd);
            // The window owns the HMENU now and destroys it on
            // WM_DESTROY; the bar's Drop must not double-free it.
            // (On a test frame with a null HWND, SetMenu fails and
            // ownership stays with the bar.)
            menubar.set_attached(!hwnd.is_null());
    ...
    /// Query-close handler (`wxCloseEvent`). Call [`CloseEvent::veto`] to
    /// cancel window destruction.
    pub fn on_query_close<F: FnMut(&crate::CloseEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_query_close = Some(Box::new(f));
    }

    /// Key-down handler (`wxKeyEvent`).
    pub fn on_key_down<F: FnMut(&crate::KeyEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_key_down.push(Box::new(f));
    }

    /// Key-up handler (`wxKeyEvent`).
    pub fn on_key_up<F: FnMut(&crate::KeyEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_key_up.push(Box::new(f));
    }

    /// Mouse handler (`wxMouseEvent`) — left/right/middle, move, wheel.
    pub fn on_mouse<F: FnMut(&crate::MouseEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_mouse.push(Box::new(f));
    }

    /// Mouse-wheel handler (`wxMouseWheelEvent`).
    pub fn on_mouse_wheel<F: FnMut(&crate::MouseWheelEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_mouse_wheel.push(Box::new(f));
    }

    /// Focus lost handler (`wxKillFocusEvent`).
    pub fn on_kill_focus<F: FnMut(&crate::KillFocusEvent) + 'static>(&self, mut f: F) {
        self.on_focus(move |ev| {
            if !ev.gained {
                f(&crate::KillFocusEvent::new(0));
            }
        });
    }

    /// Focus gained handler (`wxSetFocusEvent`).
    pub fn on_set_focus<F: FnMut(&crate::SetFocusEvent) + 'static>(&self, mut f: F) {
        self.on_focus(move |ev| {
            if ev.gained {
                f(&crate::SetFocusEvent::new(0));
            }
        });
    }

    /// Focus gained/lost handler (`wxFocusEvent`).
    pub fn on_focus<F: FnMut(&crate::FocusEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_focus.push(Box::new(f));
    }

    /// Size-change handler (`wxSizeEvent`) — richer than [`Self::on_resize`].
    pub fn on_size_event<F: FnMut(&crate::SizeEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_size_event.push(Box::new(f));
    }

    /// Position-change handler (`wxMoveEvent`).
    pub fn on_move_event<F: FnMut(&crate::MoveEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_move_event.push(Box::new(f));
    }

    /// Idle-time handler (`wxIdleEvent`).
    pub fn on_idle<F: FnMut(&mut crate::IdleEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_idle.push(Box::new(f));
    }

    /// Activation handler (`wxActivateEvent`).
    pub fn on_activate<F: FnMut(&crate::ActivateEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_activate.push(Box::new(f));
    }

    /// Character-input handler (`wxCharEvent`).
    pub fn on_char<F: FnMut(&crate::CharEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_char.push(Box::new(f));
    }

    /// Pre-dispatch key handler (`wxCharHookEvent`). Call [`CharHookEvent::veto`] to swallow the key.
    pub fn on_char_hook<F: FnMut(&crate::CharHookEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_char_hook.push(Box::new(f));
    }

    /// Background-erase handler (`wxEraseEvent`). Call [`EraseEvent::veto`] to
    /// skip the default background fill.
    pub fn on_erase<F: FnMut(&crate::EraseEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_erase = Some(Box::new(f));
    }

    /// Window-shown handler (`wxShowEvent`).
    pub fn on_show<F: FnMut(&crate::ShowEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_show.push(Box::new(f));
    }

    /// Window-hidden handler (`wxHideEvent`).
    pub fn on_hide<F: FnMut(&crate::HideEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_hide.push(Box::new(f));
    }

    /// Minimize/restore handler (`wxIconizeEvent`).
    pub fn on_iconize<F: FnMut(&crate::IconizeEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_iconize.push(Box::new(f));
    }

    /// Maximize/restore handler (`wxMaximizeEvent`).
    pub fn on_maximize<F: FnMut(&crate::MaximizeEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_maximize.push(Box::new(f));
    }

    /// Mouse-enter handler (`wxMouseEnterWindow`).
    pub fn on_mouse_enter<F: FnMut(&crate::MouseEnterEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_mouse_enter.push(Box::new(f));
    }

    /// Mouse-leave handler (`wxMouseLeaveWindow`).
    pub fn on_mouse_leave<F: FnMut(&crate::MouseLeaveEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_mouse_leave.push(Box::new(f));
    }

    /// Capture-lost handler (`wxMouseCaptureLostEvent`).
    pub fn on_capture_lost<F: FnMut(&crate::MouseCaptureLostEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_capture_lost.push(Box::new(f));
    }

    /// Mouse capture changed (`wxMouseCaptureChangedEvent`).
    pub fn on_mouse_capture_changed<F: FnMut(&crate::MouseCaptureChangedEvent) + 'static>(
        &self,
        mut f: F,
    ) {
        self.on_capture_lost(move |_| {
            f(&crate::MouseCaptureChangedEvent::new(false, 0));
        });
    }

    /// Layout query (`wxQueryLayoutEvent`).
    pub fn on_query_layout<F: FnMut(&crate::QueryLayoutEvent) + 'static>(&self, mut f: F) {
        self.on_resize(move |w, h| {
            f(&crate::QueryLayoutEvent::new(crate::Size::new(w as i32, h as i32)));
        });
    }

    /// Layout calculation (`wxCalculateLayoutEvent`).
    pub fn on_calculate_layout<F: FnMut(&crate::CalculateLayoutEvent) + 'static>(
        &self,
        mut f: F,
    ) {
        self.on_resize(move |w, h| {
            f(&crate::CalculateLayoutEvent::new(
                crate::Point::new(0, 0),
                crate::Size::new(w as i32, h as i32),
            ));
        });
    }

    /// Context-menu handler (`wxContextMenuEvent`).
    pub fn on_context_menu<F: FnMut(&crate::ContextMenuEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_context_menu.push(Box::new(f));
    }

    /// Drop-files handler (`wxDropFilesEvent`).
    pub fn on_drop_files<F: FnMut(&crate::DropFilesEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_drop_files_event.push(Box::new(f));
    }

    /// System-colour-changed handler (`wxSysColourChangedEvent`).
    pub fn on_sys_colour_changed<F: FnMut(&crate::SysColourChangedEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_sys_colour_changed.push(Box::new(f));
    }

    /// DPI-changed handler (`wxDpiChangedEvent`).
    pub fn on_dpi_changed<F: FnMut(&crate::DpiChangedEvent) + 'static>(&self, f: F) {
        self.inner.borrow_mut().on_dpi_changed.push(Box::new(f));
    }
    ...
    pub fn show(self) {
        ...
        unsafe {
            ...
            // Centre on the primary monitor's work area so the window
            // is visible even when `CreateWindowExW` used `CW_USEDEFAULT`
            // and landed off-screen on multi-monitor setups.
            ...
            // Message loop (PeekMessage so we can fire wxIdleEvent handlers).
            ...
            while running {
                if PeekMessageW(&mut msg, std::ptr::null_mut(), 0, 0, PM_REMOVE) > 0 {
                    ...
                    if ptr != 0 {
                        ...
                        let rc = Rc::from_raw(ptr as *const RefCell<FrameData>);
                        let mut handlers = std::mem::take(&mut rc.borrow_mut().on_idle);
                        drop(rc);
                        ...
                        let rc2 = Rc::from_raw(raw as *const RefCell<FrameData>);
...
impl FrameBuilder {
    ...
    /// When `false`, closing/destroying this frame does not call
    /// `PostQuitMessage` (use for MDI child frames and other
    /// auxiliary top-level windows).
    pub fn with_quit_on_destroy(mut self, quit: bool) -> Self {
        self.quit_on_destroy = quit;
        self
    }
...
unsafe extern "system" fn frame_wnd_proc(
    ...
    match msg {
        WM_NOTIFY => {
            ...
            let mut custom_draw_result: u32 = 0;
            ...
            if !nmhdr_ptr.is_null() {
                ...
                if ptr != 0 {
                    ...
                    if code == crate::controls::list_ctrl::LVN_GETDISPINFOW {
                    ...
                    } else if code == crate::controls::list_ctrl::LVN_ODCACHEHINT {
                    ...
                    } else if code == crate::controls::date_picker_ctrl::DTN_DATETIMECHANGE {
                    ...
                    } else if code as i32 == -530 || code as i32 == -520 {
                        // `TTN_GETDISPINFOW` (-530) / `TTN_GETDISPINFOA`
                        // (-520) — toolbar tooltip text request. We answer
                        // both; the handler fills the buffer using the
                        // width that matches the notification code.
                        // The NMHDR.idFrom carries the button command id,
                        // not the toolbar control id, so we can't key the
                        // lookup by `id`. Instead let every registered
                        // toolbar try to fill the request; each ignores
                        // command ids it doesn't own.
                        let mut handlers =
                            std::mem::take(&mut rc.borrow_mut().ttn_dispinfo_handlers);
                        if !handlers.is_empty() {
                            ...
                            for (k, v) in handlers {
                                data.ttn_dispinfo_handlers.entry(k).or_insert(v);
                            }
                            handled = true;
                        }
                    } else if code == crate::containers::grid::NM_CUSTOMDRAW {
                        let handler =
                            rc.borrow_mut().lv_custom_draw_handlers.remove(&id);
                        if let Some(mut h) = handler {
                            custom_draw_result = h(lparam);
                            rc.borrow_mut()
                                .lv_custom_draw_handlers
                                .insert(id, h);
                            handled = true;
        ...
        WM_COMMAND => {
            ...
            if ptr != 0 {
                ...
                // Prefer a notification-specific handler, then fall
                // back to the id-only handler (buttons, menus, …).
                let handler = rc
                    .borrow_mut()
                    .command_notify_handlers
                    .remove(&(id, notify_code));
                if let Some(mut h) = handler {
                    h();
                    rc.borrow_mut()
                        .command_notify_handlers
                        .insert((id, notify_code), h);
                } else {
        ...
        WM_CLOSE => {
            ...
            if ptr != 0 {
                ...
                if let Some(mut on_query) = rc.borrow_mut().on_query_close.take() {
                    on_query(&event);
                    rc.borrow_mut().on_query_close = Some(on_query);
                }
                if event.is_vetoed() {
                    drop(rc);
                    return 0;
                }
                if let Some(mut on_close) = rc.borrow_mut().on_close.take() {
                    on_close();
                    rc.borrow_mut().on_close = Some(on_close);
                }
                drop(rc);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        ...
        WM_SIZE => {
            ...
            if ptr != 0 {
                ...
                let size_ev = crate::SizeEvent::from_win32(wparam, lparam);
                let mut size_cbs = std::mem::take(&mut rc.borrow_mut().on_size_event);
                for cb in size_cbs.iter_mut() {
                    cb(&size_ev);
                }
                rc.borrow_mut().on_size_event = size_cbs;

                match wparam {
                    1 => {
                        let ev = crate::IconizeEvent::iconized();
                        let mut cbs = std::mem::take(&mut rc.borrow_mut().on_iconize);
                        for cb in cbs.iter_mut() {
                            cb(&ev);
                        }
                        rc.borrow_mut().on_iconize = cbs;
                    }
                    0 => {
                        let icon = crate::IconizeEvent::restored();
                        let mut icbs = std::mem::take(&mut rc.borrow_mut().on_iconize);
                        for cb in icbs.iter_mut() {
                            cb(&icon);
                        }
                        rc.borrow_mut().on_iconize = icbs;
                        let max = crate::MaximizeEvent::restored();
                        let mut mcbs = std::mem::take(&mut rc.borrow_mut().on_maximize);
                        for cb in mcbs.iter_mut() {
                            cb(&max);
                        }
                        rc.borrow_mut().on_maximize = mcbs;
                    }
                    2 | 3 => {
                        let ev = crate::MaximizeEvent::maximized();
                        let mut cbs = std::mem::take(&mut rc.borrow_mut().on_maximize);
                        for cb in cbs.iter_mut() {
                            cb(&ev);
                        }
                        rc.borrow_mut().on_maximize = cbs;
                    }
                    _ => {}
                }
        ...
        WM_ERASEBKGND => {
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let event = crate::EraseEvent::new();
                if let Some(mut on_erase) = rc.borrow_mut().on_erase.take() {
                    on_erase(&event);
                    rc.borrow_mut().on_erase = Some(on_erase);
                }
        ...
        WM_DESTROY => {
            ...
            let quit = if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let quit = rc.borrow().quit_on_destroy;
        ...
        WM_DROPFILES => {
            ...
            if ptr != 0 {
                ...
                if !paths.is_empty() {
                    ...
                    let drop_ev = crate::DropFilesEvent::new(paths.clone(), crate::Point::new(x, y));
                    let handler = rc.borrow_mut().drop_files_handler.take();
                    if let Some(mut h) = handler {
                        h(DroppedFiles::new(paths));
                        rc.borrow_mut().drop_files_handler = Some(h);
                    }
                    let mut ev_handlers = std::mem::take(&mut rc.borrow_mut().on_drop_files_event);
                    drop(rc);
                    for cb in ev_handlers.iter_mut() {
                        cb(&drop_ev);
                    }
                    let raw = GetWindowLongPtrW(hwnd, GWLP_USERDATA);
                    unsafe {
                    ...
                    let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                    rc2.borrow_mut().on_drop_files_event = ev_handlers;
                    drop(rc2);
                } else {
                    drop(rc);
                }
        ...
        WM_CONTEXTMENU => {
            ...
            let is_keyboard = x < 0 || y < 0;
            let ev = if is_keyboard {
                crate::ContextMenuEvent::keyboard(crate::Point::new(0, 0))
            } else {
                crate::ContextMenuEvent::mouse(crate::Point::new(x, y))
            };
            let ptr = GetWindowLongPtrW(hwnd, GWLP_USERDATA);
            if ptr != 0 {
                unsafe {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let mut handlers = std::mem::take(&mut rc.borrow_mut().on_context_menu);
                drop(rc);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_context_menu = handlers;
                drop(rc2);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        }
        WM_SYSCOLORCHANGE => {
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let ev = crate::SysColourChangedEvent::new();
                let mut handlers = std::mem::take(&mut rc.borrow_mut().on_sys_colour_changed);
                drop(rc);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_sys_colour_changed = handlers;
                drop(rc2);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        }
        0x02E0 /* WM_DPICHANGED */ => {
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let old_dpi = crate::get_dpi_for_window(hwnd).value();
                let ev = crate::DpiChangedEvent::new(old_dpi, new_dpi, suggested);
                let mut handlers = std::mem::take(&mut rc.borrow_mut().on_dpi_changed);
                drop(rc);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_dpi_changed = handlers;
                drop(rc2);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        }
        WM_KEYDOWN | WM_SYSKEYDOWN => {
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let ctrl = unsafe { GetKeyState(0x11) } & 0x8000u16 as i16 != 0;
                let ev = crate::KeyEvent::from_win32(wparam as u32, lparam, msg == WM_SYSKEYDOWN)
                    .with_control_state(ctrl);
                let mut handlers = std::mem::take(&mut rc.borrow_mut().on_key_down);
                drop(rc);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_key_down = handlers;
                drop(rc2);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        }
        WM_KEYUP | WM_SYSKEYUP => {
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let ctrl = unsafe { GetKeyState(0x11) } & 0x8000u16 as i16 != 0;
                let ev = crate::KeyEvent::from_win32(wparam as u32, lparam, msg == WM_SYSKEYUP)
                    .with_control_state(ctrl);
                let mut handlers = std::mem::take(&mut rc.borrow_mut().on_key_up);
                drop(rc);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_key_up = handlers;
                drop(rc2);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        }
        0x02A3 /* WM_MOUSELEAVE */ => {
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                rc.borrow_mut().mouse_tracking = false;
                let ev = crate::MouseLeaveEvent::new();
                let mut handlers = std::mem::take(&mut rc.borrow_mut().on_mouse_leave);
                drop(rc);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_mouse_leave = handlers;
                drop(rc2);
            }
            0
        }
        0x0215 /* WM_CAPTURECHANGED */ => {
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let ev = crate::MouseCaptureLostEvent::new();
                let mut handlers = std::mem::take(&mut rc.borrow_mut().on_capture_lost);
                drop(rc);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_capture_lost = handlers;
                drop(rc2);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        }
        ...
        | WM_MBUTTONDOWN | WM_MBUTTONUP | WM_MOUSEMOVE | WM_MOUSEWHEEL => {
            ...
            let kind = match msg {
                ...
                WM_RBUTTONDOWN => MouseEventKind::RightDown,
                WM_RBUTTONUP => MouseEventKind::RightUp,
                WM_MBUTTONDOWN => MouseEventKind::MiddleDown,
                WM_MBUTTONUP => MouseEventKind::MiddleUp,
                WM_MOUSEMOVE => MouseEventKind::Move,
                WM_MOUSEWHEEL => MouseEventKind::Wheel,
                _ => MouseEventKind::Move,
            };
            let ev = crate::MouseEvent::from_win32(kind, lparam, wparam);
            let ptr = GetWindowLongPtrW(hwnd, GWLP_USERDATA);
            if ptr != 0 {
                unsafe {
                    Rc::increment_strong_count(ptr as *const RefCell<FrameData>);
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                if msg == WM_MOUSEMOVE {
                    let need_track = !rc.borrow().mouse_tracking
                        && (!rc.borrow().on_mouse_enter.is_empty()
                            || !rc.borrow().on_mouse_leave.is_empty());
                    if need_track {
                        ...
                        if unsafe { TrackMouseEvent(&mut tme) } != 0 {
                            rc.borrow_mut().mouse_tracking = true;
                            let enter = crate::MouseEnterEvent::new(ev.position);
                            let mut enter_handlers =
                                std::mem::take(&mut rc.borrow_mut().on_mouse_enter);
                            for h in enter_handlers.iter_mut() {
                                h(&enter);
                            }
                            rc.borrow_mut().on_mouse_enter = enter_handlers;
                        }
                    }
                }
                drop(rc);
                let raw2 = GetWindowLongPtrW(hwnd, GWLP_USERDATA);
                unsafe {
                    Rc::increment_strong_count(raw2 as *const RefCell<FrameData>);
                }
                let rc2 = unsafe { Rc::from_raw(raw2 as *const RefCell<FrameData>) };
                if msg == WM_MOUSEWHEEL {
                    let wheel = crate::MouseWheelEvent::new(ev.position, ev.wheel_delta);
                    let mut wheel_handlers = std::mem::take(&mut rc2.borrow_mut().on_mouse_wheel);
                    for h in wheel_handlers.iter_mut() {
                        h(&wheel);
                    }
                    rc2.borrow_mut().on_mouse_wheel = wheel_handlers;
                }
                let mut handlers = std::mem::take(&mut rc2.borrow_mut().on_mouse);
                drop(rc2);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc4 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc4.borrow_mut().on_mouse = handlers;
                drop(rc4);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        }
        WM_SETFOCUS | WM_KILLFOCUS => {
            let ev = if msg == WM_SETFOCUS {
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let mut handlers = std::mem::take(&mut rc.borrow_mut().on_focus);
                drop(rc);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_focus = handlers;
                drop(rc2);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        }
        WM_SHOWWINDOW => {
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                if show {
                    let mut handlers = std::mem::take(&mut rc.borrow_mut().on_show);
                    drop(rc);
                    let ev = crate::ShowEvent::new();
                    for h in handlers.iter_mut() {
                        h(&ev);
                    }
                    let raw = GetWindowLongPtrW(hwnd, GWLP_USERDATA);
                    ...
                    let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                    rc2.borrow_mut().on_show = handlers;
                    drop(rc2);
                } else {
                    let mut handlers = std::mem::take(&mut rc.borrow_mut().on_hide);
                    drop(rc);
                    let ev = crate::HideEvent::new();
                    for h in handlers.iter_mut() {
                        h(&ev);
                    }
                    let raw = GetWindowLongPtrW(hwnd, GWLP_USERDATA);
                    ...
                    let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                    rc2.borrow_mut().on_hide = handlers;
                    drop(rc2);
                }
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        ...
        WM_ACTIVATE => {
            let ev = crate::ActivateEvent::from_win32(wparam);
            let ptr = GetWindowLongPtrW(hwnd, GWLP_USERDATA);
            if ptr != 0 {
                unsafe {
                    Rc::increment_strong_count(ptr as *const RefCell<FrameData>);
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let mut handlers = std::mem::take(&mut rc.borrow_mut().on_activate);
                drop(rc);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_activate = handlers;
                drop(rc2);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        }
        WM_CHAR => {
            ...
            let hook_ev = crate::CharHookEvent::new(code);
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let mut hook_handlers = std::mem::take(&mut rc.borrow_mut().on_char_hook);
                drop(rc);
                for h in hook_handlers.iter_mut() {
                    h(&hook_ev);
                }
                vetoed = hook_ev.is_vetoed();
                let raw = GetWindowLongPtrW(hwnd, GWLP_USERDATA);
                unsafe {
                    Rc::increment_strong_count(raw as *const RefCell<FrameData>);
                }
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_char_hook = hook_handlers;
                if !vetoed {
                    let ev = crate::CharEvent::from_win32(code, false);
                    let mut handlers = std::mem::take(&mut rc2.borrow_mut().on_char);
                    drop(rc2);
                    for h in handlers.iter_mut() {
                        h(&ev);
                    }
                    ...
                    let rc3 = unsafe { Rc::from_raw(raw2 as *const RefCell<FrameData>) };
                    rc3.borrow_mut().on_char = handlers;
                    drop(rc3);
                } else {
                    drop(rc2);
                }
        ...
        WM_MOVE => {
            let ev = crate::MoveEvent::from_win32(lparam);
            let ptr = GetWindowLongPtrW(hwnd, GWLP_USERDATA);
            if ptr != 0 {
                unsafe {
                    Rc::increment_strong_count(ptr as *const RefCell<FrameData>);
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let mut handlers = std::mem::take(&mut rc.borrow_mut().on_move_event);
                drop(rc);
                for h in handlers.iter_mut() {
                    h(&ev);
                }
                ...
                let rc2 = unsafe { Rc::from_raw(raw as *const RefCell<FrameData>) };
                rc2.borrow_mut().on_move_event = handlers;
                drop(rc2);
            }
            DefWindowProcW(hwnd, msg, wparam, lparam)
        }
        WM_TIMER => {
            ...
            if ptr != 0 {
                ...
                let rc = unsafe { Rc::from_raw(ptr as *const RefCell<FrameData>) };
                let msg_id = rc.borrow().timer_tick_routes.get(&timer_id).copied();
                drop(rc);
                if let Some(msg_id) = msg_id {
                    // SAFETY: `hwnd` is live; posting to the same thread's queue.
                    unsafe {
...
mod tests {
    ...
    use super::*;
    use crate::core::accelerator::Accelerator;
    use crate::window::menu::{Menu, MenuBar};

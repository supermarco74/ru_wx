...
with no sizer, use a [`StaticBox`](../controls/static_box.md).

## See Also
- [`frame.rs`](frame.md) — typical parent of a top-level `Panel`; owns the dispatch tables
- [`sizer.rs`](../containers/sizer.md) — `BoxSizer` is the standard layout engine inside a panel
- [`dialog.rs`](dialog.md) — modal/modeless dialog uses a similar WndProc pattern
- [`static_box.rs`](../controls/static_box.md) — purely decorative alternative; does not host a sizer

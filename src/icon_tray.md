...
- [frame](frame.md) — every tray icon is owned by a frame. The frame's `wndproc` dispatches
  `WM_APP + n` messages to the registered tray handlers.
- [menu](menu.md) — `Menu` is the context-menu payload for `set_menu`.
- [icon](icon.md) — `svg_bytes_to_hicon` / `destroy_hicon` are the building blocks for the
  tray `HICON`.
- [timer](timer.md) — for periodic re-pop of tray icons / status updates.
- [prelude](prelude.md)

...
mod imp {
    ...
    use windows_sys::Win32::UI::WindowsAndMessaging::{
        CreateWindowExW, FindWindowExW, GetAncestor, SendMessageW, CW_USEDEFAULT, GA_ROOT, WM_USER,
        WS_POPUP,
    };
    ...
    const TTM_ADDTOOL: u32 = WM_USER + 4; // 0x0404
    const TTM_DELTOOL: u32 = WM_USER + 5; // 0x0405
    const TTM_UPDATETIPTEXT: u32 = WM_USER + 12; // 0x040C
    ...
    unsafe fn attach_to_hwnd(target: HWND, text: &str) {
        ...
        let info = fill_toolinfo(target, wide_text.as_ptr());
    ...
    unsafe fn update_tip_text(target: HWND, text: &str) {
        let top = GetAncestor(target, GA_ROOT);
        let tip = find_or_create_tooltip_for(top);
        if tip.is_null() {
            return;
    ...
    impl ToolTip {
        ...
        pub fn set_text(&self, text: &str) {
            ...
            if let Some(hwnd) = inner.target_hwnd {
                ...
                unsafe { update_tip_text(hwnd, &inner.text) };
            }
        }
        ...
        pub fn attach(&self, target: &WidgetRef) {
            ...
            // SAFETY: Win32 FFI call with validated arguments (HWND / HMENU / handle) and a buffer large enough for the output.
            unsafe { attach_to_hwnd(hwnd, &inner.text) };
        }
...
mod imp {
    ...
    impl ToolTip {
        ...
        pub fn attach(&self, _target: &crate::widget::WidgetRef) {}
        pub fn enable(_enabled: bool) {}
        pub fn detach(_target: &crate::widget::WidgetRef) {}
    }
}

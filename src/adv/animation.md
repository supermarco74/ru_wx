...
- Decoded frames are stored as RGBA8 in an [`Image`](../dc/image.md); the same buffer is reused on every blit. Each repaint converts the current frame to a 32-bit DIB and `BitBlt`s / `StretchBlt`s it into the control's HDC.
...
- [`image.md`](../dc/image.md) — `Image` is the per-frame pixel buffer.
- [`bitmap.md`](../dc/bitmap.md) — `Image::to_bitmap` is the conversion used during `WM_PAINT`.

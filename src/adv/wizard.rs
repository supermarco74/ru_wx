...
impl Wizard {
    ...
    /// Register a callback with a [`WizardEvent`] payload.
    pub fn on_wizard_event<F: FnMut(&crate::core::wizard_event::WizardEvent) + 'static>(
        &mut self,
        f: F,
    ) {
        let mut handler = f;
        self.on_page_changed(move |idx| {
            handler(&crate::core::wizard_event::WizardEvent::new(
                crate::core::wizard_event::WizardEventKind::PageChanged,
                idx,
            ));
        });
    }

...
impl PropertyGrid {
    ...
    pub fn new(frame: &Frame) -> Self {
        ...
        let data = Rc::new(RefCell::new(PropertyGridData {
            ...
            id,
            rect: crate::core::geometry::Rect::new(0, 0, 200, 200),
            properties: Vec::new(),
    ...
    /// Return the name of the property at `idx`, or `None` if out of range.
    ...
    /// Register a callback with a [`PropertyGridEvent`] payload.
    pub fn on_property_grid_event<F: FnMut(&crate::core::property_grid_event::PropertyGridEvent) + 'static>(
        &mut self,
        f: F,
    ) {
        let mut handler = f;
        self.on_change(move |idx| {
            handler(&crate::core::property_grid_event::PropertyGridEvent::new(idx, true));
        });
    }
...
fn paint(data: &PropertyGridData, hdc: HDC, rect: RECT) {
    ...
    unsafe {
        // Fill the background.
        let bg_brush = CreateSolidBrush(crate::core::geometry::Colour::new(255, 255, 255, 0).to_colorref());
        ...
        let pen = CreatePen(PS_SOLID, 1, crate::core::geometry::Colour::new(180, 180, 180, 0).to_colorref());
        if pen.is_null() {
        ...
        SetBkMode(hdc, 1);
        let text_colour = crate::core::geometry::Colour::new(0, 0, 0, 0).to_colorref();
...
impl ToColorref for crate::core::geometry::Colour {
    fn to_colorref(self) -> u32 {
    ...
    let editor_hwnd = unsafe {
        match prop_value {
            ...
            PropertyValue::Choice { options, selected } => {
                ...
                SendMessageW(hwnd_combo, CB_SETCURSEL, selected, 0);
                SetFocus(hwnd_combo);
                hwnd_combo
            }
        }
    ...
    let new_value: Option<PropertyValue> = unsafe {
        match old_value {
            ...
            PropertyValue::Choice { options, .. } => {
                // `CB_GETCURSEL` returns `CB_ERR` (-1) when nothing
                // is selected; check before the unsigned cast (on
                // Win64 `-1 as usize` would be `usize::MAX`, which
                // the `< options.len()` guard catches only by luck).
                let sel = SendMessageW(editor.hwnd, CB_GETCURSEL, 0, 0);
    ...
    let result = match msg {
        ...
        WM_COMMAND => {
            ...
            match notification {
                ...
                BN_CLICKED => {
                    // Boolean toggle: commit if the click came from
                    // the active editor. `WM_COMMAND`'s `lparam` is
                    // the HWND of the child control that sent the
                    // notification, so compare it with the editor's
                    // HWND (the low word of `wparam` is the control
                    // *ID*, which never matches an HWND). Use
                    // `try_borrow` to be re-entrancy safe: a sizer
                    // layout in progress may hold the borrow.

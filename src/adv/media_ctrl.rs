...
impl MediaCtrl {
    ...
    pub fn load(&self, path: &Path) -> Result<(), String> {
        ...
        let cmd = mci_open_command(path_str, &self.inner.borrow().alias);
        ...
        match res {
            Ok(()) => {
                ...
                // MP3/WAV length and position queries need an explicit
                // time format; without this MCI often returns empty replies
                // and the UI progress bar stays frozen.
                let tf = format!("set {} time format milliseconds", inner.alias);
                let _ = mci_send(&tf, None);
                Ok(())
            }
            Err(e) => {
    ...
    pub fn length_ms(&self) -> Option<u64> {
        ...
        let alias = inner.alias.clone();
        drop(inner);
        mci_query_ms(&alias, "length")
    }

    /// Refresh cached playback state from the MCI device (`status mode`).
    pub fn sync_state(&self) {
        ...
        let alias = inner.alias.clone();
        drop(inner);
        if let Some(mode) = mci_query_text(&alias, "mode") {
            let mut inner = self.inner.borrow_mut();
            inner.state = match mode.to_ascii_lowercase().as_str() {
                "playing" => MciState::Playing,
                "paused" | "paused recording" => MciState::Paused,
                "stopped" | "not ready" => MciState::Stopped,
                _ => inner.state,
            };
        }
    }
    ...
    pub fn seek_ms(&self, ms: u64) -> Result<(), String> {
        let was_playing = self.inner.borrow().state == MciState::Playing;
        let cmd = {
            let inner = self.inner.borrow();
            if !inner.has_media {
                return Err("no media loaded".to_string());
        ...
        mci_send(&cmd, None)?;
        // MCI `seek` stops playback; resume automatically if we were playing.
        if was_playing {
            let play_cmd = format!("play {}", self.inner.borrow().alias);
            mci_send(&play_cmd, None)?;
            self.inner.borrow_mut().state = MciState::Playing;
        }
        Ok(())
    }
...
#[cfg(target_os = "windows")]
fn mci_open_command(path_str: &str, alias: &str) -> String {
    let escaped = escape_for_mci(path_str);
    ...
    match ext.as_str() {
        "mp3" | "wma" | "mpeg" | "mpg" => {
            format!("open \"{escaped}\" type MPEGVideo alias {alias}")
        }
        "wav" => format!("open \"{escaped}\" type waveaudio alias {alias}"),
        _ => format!("open \"{escaped}\" alias {alias}"),
    }
}

#[cfg(target_os = "windows")]
fn mci_query_text(alias: &str, field: &str) -> Option<String> {
    let cmd = format!("status {alias} {field}");
    let mut buf = [0u16; 128];
    mci_send(&cmd, Some(&mut buf)).ok()?;
    let len = buf.iter().position(|&c| c == 0).unwrap_or(buf.len());
...
#[cfg(target_os = "windows")]
fn mci_query_ms(alias: &str, field: &str) -> Option<u64> {
    mci_query_text(alias, field).and_then(|s| parse_mci_time_ms(&s))
}

#[cfg(target_os = "windows")]
fn mci_query_position_ms(alias: &str) -> Option<u64> {
    mci_query_ms(alias, "position").or_else(|| {
        let cmd = format!("where {alias} position");
        let mut buf = [0u16; 128];
        mci_send(&cmd, Some(&mut buf))
            .ok()
            .and_then(|_| wide_buf_to_string(&buf))
            .and_then(|s| parse_mci_time_ms(&s))
    })
}
...
/// Parse MCI time replies: plain ms, `12345 milliseconds`, `mm:ss`, `h:mm:ss`.
#[cfg(target_os = "windows")]
fn parse_mci_time_ms(s: &str) -> Option<u64> {
    let trimmed = s.trim();
...
fn parse_u64_from_wide(buf: &[u16]) -> Option<u64> {
    wide_buf_to_string(buf).and_then(|s| parse_mci_time_ms(&s))
}
...
mod tests {
    ...
    #[test]
    fn parse_mci_mm_ss() {
        assert_eq!(parse_mci_time_ms("02:15"), Some(135_000));
    }

    #[test]
    fn parse_mci_with_unit_suffix() {
        assert_eq!(parse_mci_time_ms("45230 milliseconds"), Some(45_230));
    }

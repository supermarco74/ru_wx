...
- `GWLP_USERDATA` stores the raw `Rc<RefCell<…>>` pointer; the WndProc reconstructs an owned `Rc` for the duration of the arm and releases the extra strong count on drop. This is the same re-entrancy-safe pattern used by [`frame.md`](../window/frame.md).
...
- [`image.md`](../dc/image.md) — frame pixels are stored as `Image`.
- [`bitmap.md`](../dc/bitmap.md) — `Image::to_bitmap` is the conversion used in `WM_PAINT`.
- [`widget.md`](../core/widget.md) — `Window` trait, `as_widget_ref`.

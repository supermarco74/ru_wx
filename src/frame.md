...
- [`accelerator.rs`](./accelerator.md) — `Accelerator` type used in `register_accelerator`
- [`sizer.rs`](./sizer.md) — `BoxSizer` passed to `set_sizer`
- [`drop_target.rs`](./drop_target.md) — `DroppedFiles` and Shell-level dispatch helpers
- [`ole_dnd.rs`](./ole_dnd.md) — `OleDropTarget`, `OleDroppedData`, `OleDropError`, `OleDropPosition`
- [`icon_tray.rs`](./icon_tray.md) — uses `tray_message_handlers` for `NIN_*` notifications
- [`date_picker_ctrl.rs`](./date_picker_ctrl.md) — uses `dtn_handlers` for `DTN_DATETIMECHANGE`
- [`list_ctrl.rs`](./list_ctrl.md) — uses `disp_info_handlers` for `LVS_OWNERDATA`
- [`status_bar.rs`](./status_bar.md) — uses `add_resize_handler` to reapply field widths on `WM_SIZE`

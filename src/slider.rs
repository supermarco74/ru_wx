...
//!
//! Changes are reported as `WM_HSCROLL` / `WM_VSCROLL`, which the
//! Win32 subsystem synthesises as `WM_COMMAND` messages with
//! notification code `EN_CHANGE` (0x300) actually — no, the
//! `HIWORD(wparam)` of the synthesised `WM_COMMAND` is the SB_*
//! scroll-bar code (e.g. `SB_THUMBPOSITION`, `SB_ENDSCROLL`).
//!
//! We register a `WM_COMMAND` handler on the frame for the slider's
//! id; the handler is called for every scroll event, and
//! `get_value()` returns the current thumb position.
...
#[cfg(target_os = "windows")]
const TBM_GETPOS: u32 = 0x0400 + 21; // TBM_GETPOS = WM_USER
#[cfg(target_os = "windows")]
...
#[cfg(target_os = "windows")]
const TBM_SETRANGEMIN: u32 = 0x0400 + 3; // fRedraw = wparam
#[cfg(target_os = "windows")]
const TBM_SETRANGEMAX: u32 = 0x0400 + 4;
#[cfg(target_os = "windows")]
const TBM_SETTICFREQ: u32 = 0x0400 + 20;
#[cfg(target_os = "windows")]
const TBM_SETLINESIZE: u32 = 0x0400 + 23;
#[cfg(target_os = "windows")]
const TBM_SETPAGESIZE: u32 = 0x0400 + 25;
#[cfg(target_os = "windows")]
#[allow(dead_code)] // Win32 ABI surface — base for TBM_* message codes
const WM_USER: u32 = 0x0400;

// TBM_GETPOS is defined as (WM_USER); TBM_SETPOS is (WM_USER+5) etc.
// We re-derive them here using the WM_USER base to keep the table
// self-contained and version-stable.
const _: () = (); // keep file-level consts tidy
...
impl Slider {
    ...
    pub fn on_value_change<F: FnMut() + 'static>(&self, frame: &Frame, mut callback: F) {
        ...
        frame.register_command_handler(
            id,
            Box::new(move || {
                // Sync our cached value with the control's actual position.
                #[cfg(target_os = "windows")]
                {

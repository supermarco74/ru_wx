...
a "notebook of pages", each backed by a [`Panel`](../window/panel.md).
...
- [`panel.md`](../window/panel.md) — page content container; the `Tab` keeps a
  cloned `Panel` per page so it can re-layout on resize.
- [`frame.md`](../window/frame.md) — supplies the parent HWND and the
  `register_notify_handler` mechanism.
- [`image_list.md`](../dc/image_list.md) — optional source of tab icons.

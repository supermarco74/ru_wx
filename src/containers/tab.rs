...
// Use the **wide** tab-control message variants. `TCM_FIRST + 5/6/7`
// are the ANSI (`*A`) forms; passing a `TCITEMW` to them makes the
// control read `pszText` as narrow chars, so UTF-16 labels show only
// their first letter (the high byte of the second wchar is 0x00).
#[cfg(target_os = "windows")]
const TCM_INSERTITEM: u32 = TCM_FIRST + 62; // TCM_INSERTITEMW
#[cfg(target_os = "windows")]
#[allow(dead_code)]
...
/// `TCM_GETITEMW` — retrieve information about an existing tab.
#[cfg(target_os = "windows")]
const TCM_GETITEM: u32 = TCM_FIRST + 60; // TCM_GETITEMW
/// `TCM_SETITEMW` — update one or more fields of an existing tab.
#[cfg(target_os = "windows")]
const TCM_SETITEM: u32 = TCM_FIRST + 61; // TCM_SETITEMW
    ...
    pub fn set_selection(&self, index: usize) {
        #[cfg(target_os = "windows")]
        {
            let selected_page = {
                ...
                inner.page_panels[index].clone()
            };

            // Re-layout outside the mutable borrow so `layout_page` can
            // read `TabInner` without a RefCell conflict.
            let inner = self.inner.borrow();
            inner.layout_page(&selected_page);
        }
        #[cfg(not(target_os = "windows"))]
        {
            let _ = index;
        ...
        frame.register_notify_handler(
            ...
            Box::new(move |_code| {
                ...
                unsafe {
                    ...
                    // `TCM_GETCURSEL` returns -1 when no tab is
                    // selected; bail out before the unsigned cast
                    // (on Win64 it would become `usize::MAX`).
                    if sel < 0 {
                        return;
                    }
                    let new_selected = sel as usize;
...
mod tests {
    ...
    #[test]
    fn tcm_constants_use_wide_variants() {
        assert_eq!(TCM_INSERTITEM, TCM_FIRST + 62);
        assert_eq!(TCM_GETITEM, TCM_FIRST + 60);
        assert_eq!(TCM_SETITEM, TCM_FIRST + 61);
    }
}

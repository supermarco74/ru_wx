...
- [frame](frame.md) — typically the parent. The control's `id` is registered into the frame's
  command dispatch so the click reaches the `on_change` callback.
- [panel](panel.md) — also a valid parent if commands are forwarded.
- [sizer](sizer.md) — `as_widget_ref()` to insert into a `BoxSizer`.

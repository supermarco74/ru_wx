...
//! Tests if StaticText alone triggers the bug.
#![windows_subsystem = "windows"]
use ru_wx::{App, Button, Frame, StaticText, StatusBar};
fn main() {
    let _app = App::new();
    ...
    let _lbl = StaticText::new(&frame, "Type some text, then click \"Set status\":");
    let s = status.clone();
